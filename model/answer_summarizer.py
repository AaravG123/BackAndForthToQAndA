...
answers = [preprocessed_data[f"pair{i+1}"][f"answer{i+1}"] for i in range(100, 130)]
ground_truth_summaries = [summarized_data[f"pair{i+1}"][f"answer{i+1}"] for i in range(100, 130)]

rouge_scores = []
generated_summaries = []

for i in range(0, len(answers)):
    inputs = tokenizer(answers[i], return_tensors="pt", max_length=1024, padding='max_length', truncation=True)
